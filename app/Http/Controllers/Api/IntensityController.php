...
class IntensityController extends Controller
{
    public function store(Request $request) // Ini adalah fungsi / method untuk menyimpan data temperatur ke dalam database
    {
        ...
        $intensity = Intensity::create($request->all()); // Menyimpan data temperatur ke dalam database

        return response()->json($intensity, 201); // Memberikan response berupa data temperatur yang telah disimpan ke dalam database
    }
}
